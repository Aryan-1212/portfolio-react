...
function ProjectCard({title, to, tech, desc, src}) {
    return (
            <div className='flex flex-col p-10 rounded-lg custom-shadow'>
              <div>
                <Link to={to}><img className='rounded-lg shadow-2xl duration-500 hover:p-2' src={src} alt="" /></Link>
              </div>
              <div className='flex space-y-4 flex-col py-5'>
                <div><Link to={to} className='text-5xl text-orange-700 font-bold'>{title}</Link></div>
